...
// From lowest to highest. Depends on C assigning these numbers in order.
typedef enum {
  PREC_NONE,
  PREC_ASSIGNMENT, // =
  PREC_OR,         // or
...
static void namedVariable(Token name, bool canAssign) {
  ...
  if (arg != -1) {
    ...
    // If a variable is referenced in a function and we can't resolve it
    // locally, look for it in upvalues
...
static void parsePrecedence(Precedence precedence) {
  ...
  // Look up prefix parser for the current token. It should *always* belong
  // to *some* prefix expression - ie, grouping, unary, number etc.
  ...
  // Now look up infix parsers - ie, binary
...
static int addUpvalue(Compiler *compiler, uint8_t index, bool isLocal) {
  ...
  // If an upvalue already exists for that index, just use that
...
static void function(FunctionType type) {
  ...
  // OP_CLOSURE takes a variable number of bytes, two for each upvalue
...
static void forStatement() {
  ...
  statement();
  // This actually loops back to the *increment* if one is defined
  emitLoop(loopStart);
